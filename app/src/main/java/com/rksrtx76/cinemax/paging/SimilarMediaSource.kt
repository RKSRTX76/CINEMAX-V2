...
class SimilarMediaSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Media> {
        return try {
            ...
            val similarMedia = if(mediaType == MediaType.MOVIE){
                api.getSimilarMovies(page = nextPage, movieId = mediaId)
            }else{
                api.getSimilarTvShows(page = nextPage, showId = mediaId)
            }

            LoadResult.Page(
                data = similarMedia.results,
                prevKey = if(nextPage == 1) null else nextPage - 1,
                nextKey = if(similarMedia.results.isEmpty()) null else similarMedia.page + 1
            )
        }catch (e : IOException){
